...
test('AC-F：同一球員選過後不再出現在其他卡片的球員下拉（#3 去重）', async ({ page }) => {
  await registerAndTeam(page, 'm4dup')
  await addPlayers(page, ['DupA', 'DupB'])
  await gotoLineup(page, '2026-07-13', 'Foxes')

  const starterCol = page.locator('.roster-col[data-col="starter"]')

  // 第一張卡選 DupA
  await page.getByRole('button', { name: '＋ 直接加入先發' }).click()
  const card1 = starterCol.locator('.roster-card').nth(0)
  await card1.getByLabel('球員').selectOption({ label: 'DupA' })

  // 第二張卡：DupA 已被別張卡選走 → 不再出現在下拉；DupB 仍在
  await page.getByRole('button', { name: '＋ 直接加入先發' }).click()
  const card2 = starterCol.locator('.roster-card').nth(1)
  await expect(card2.getByLabel('球員').locator('option', { hasText: /^DupA$/ })).toHaveCount(0)
  await expect(card2.getByLabel('球員').locator('option', { hasText: /^DupB$/ })).toHaveCount(1)

  // 第一張卡仍保有自己選的 DupA（不會把自己選的也藏掉）
  await expect(card1.getByLabel('球員').locator('option', { hasText: /^DupA$/ })).toHaveCount(1)
})

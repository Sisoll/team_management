...
導覽模型＝**逐層鑽入 + 麵包屑返回**（非全域切換器）。

### 4.1 第 1 層 · 全域（個人視角）
- 頂欄：`⚾ 紀錄台`（品牌，點擊回我的球隊） + `👤 <顯示名稱> ▾`（帳號選單：登出）。
- **不放球隊切換器**（登入即個人視角；屬哪隊的哪個角色，進球隊後才看）。
- 落地頁＝**我的球隊**。
- 個人首頁 / 個人行事曆 / 個人統計 → **之後需要再加，現在不放全域分頁列**。

### 4.2 第 2 層 · 球隊內
- 進球隊後顯示麵包屑 `我的球隊 › <球隊名>` + **「你的身分：<角色>」chip**。
- 分頁列（全部畫出、未做的 disabled「即將推出」）：
...
| 記錄（逐打席 / 換人） | 佔位（M3） |
| 計分板（SSE 即時） | 佔位（M3） |
| 數據（單場 box score） | 佔位（M3） |
| 時間線（事件流） | 佔位（M3） |

> **佔位行為**：disabled 分頁不可點（或點擊顯示「即將推出」提示），不建立對應路由內容；僅渲染一個佔位畫面。
...
```
/                                   → 我的球隊（落地）
/teams/:teamId                      → 轉址 /teams/:teamId/players
/teams/:teamId/*  (TeamLayout: 外殼＋球隊分頁列)
    players                         ✅ 球員名冊
    games                          ✅ 賽程列表
    games/new                      ✅ 建立比賽
    overview | calendar | stats | settings   → 佔位「即將推出」
/games/:gameId                      → 轉址 /games/:gameId/lineup
/games/:gameId/*  (GameLayout: 外殼＋比賽分頁列)
    info                           ✅ 比賽資訊
    lineup                         ✅ 出賽名單
    record | scoreboard | box | timeline     → 佔位「即將推出」
```

- `TeamLayout` / `GameLayout` 載入該球隊/比賽基本資料、渲染麵包屑與分頁列、以 `<Outlet/>` 掛分頁內容。
- 既有頁面元件拆成分頁內容（如把 `TeamPage` 的「球員表」與「比賽列表」分別歸到 `players` / `games` 分頁）。

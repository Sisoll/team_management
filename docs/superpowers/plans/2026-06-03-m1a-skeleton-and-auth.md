...
**Tech Stack:** Java 17 · Spring Boot 3.3 · Maven · Spring Security 6 + JWT(jjwt 0.12) · Spring Data JPA · Flyway · PostgreSQL 16 · Testcontainers · React 18 + Vite + TypeScript · Playwright
...
| 項目 | 選擇 |
|---|---|
| 建置工具 / JDK | Maven · Java 17（LTS，本機現有 Zulu 17） |
...
- [ ] **Step 1: 安裝 Playwright** — `cd frontend && npm i -D @playwright/test && npx playwright install chromium`
...
- [ ] **Step 4: 跑 E2E（後端 + DB 需在跑）**

Run: `cd backend && podman compose up -d db && ./mvnw -q spring-boot:run &` 然後 `cd frontend && npx playwright test`
Expected: PASS（完整端到端：前端→後端→真 Postgres）。
